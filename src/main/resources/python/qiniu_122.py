...
list_url = "https://home.meishichina.com/recipe/bing/page/"
sort_id=18
# 获取分类下的所有菜单 id
...
#头像
avater_xpath="//a[@class='uright']/img/@src"
#获取作者
...
def upload(imgurl,imgname):
    ...
    s= "http://q6d69wjq6.bkt.clouddn.com/"+ss["key"]
    return s
...
if __name__ == '__main__':
    avater = "https://i5.meishichina.com/data/avatar/011/86/16/27_avatar_big.jpg?x-oss-process=style/c320&v=20200301"
    avater_qiniu=upload(avater,888)
    print(avater_qiniu)
    ...
    #         avater=xmlDecode(url,avater_xpath)[0]
    #
    #         avater_qiniu=upload(avater,data_id[i])
    #         # material =json.dumps(dict(zip(xmlDecode(url,lax_xpath),xmlDecode(url,lay_xpath))),ensure_ascii=False)
    #
    #         addUser_sql="insert  into user (username,avater) values('%s','%s')"%(user,avater_qiniu)
    #
    #         # addMenu_sql="insert into menu (title,userid,step,material,img,sortid,state) values('%s','%d','%s','%s','%s','%d','%d')" %(title,random.randint(1,100),step,material,avater_qiniu,sort_id,0)
    #         print(addUser_sql)
