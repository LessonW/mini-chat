...
list_url = "https://home.meishichina.com/recipe/chuancai/page/"
sort_id=18
# 获取分类下的所有菜单 id
...
def uploadAvatar(imgurl,imgname):
    access_key = 'GOuhUINKN0-LhA6ANW2cj_d-aJ-KEk6VUz_bQmCp'
    secret_key = '99Fwz7a_u1DKue2hqVRGx_ct174_pAUiXOIk9nOi'
    bucket_name = 'mini-avatar'
    q = Auth(access_key, secret_key)
    bucket = BucketManager(q)
    ret, info = bucket.fetch(imgurl, bucket_name, imgname)
    # print(info.text_body)
    ss= json.loads(info.text_body)
    s= "http://q6irmt9u0.bkt.clouddn.com/"+ss["key"]
    return s
...
if __name__ == '__main__':
    for i in range(2,20):
        ...
        for i in range(0, len(data_id)):
            ...
            avater_qiniu=uploadAvatar(avater,data_id[i])
